...
const checkLoginStatues = async () => {
    const loginButton = await page.$('.login____RTRY');
    if (loginButton == null) {
        login = true;
        return;
    }
...
const askQuestion = async () => {
    ...
    const inputSelector = '[data-testid="msh-chatinput-editor"]';
...
const updateQuestion = async () => {
    ...
    const answers = await page.$$('.content___hMdwe');
    if (answers.length <= 0) {
        return;
    }
...
const completeQuestion = async () => {
    ...
    const generatingButton = await page.$('.stop___Cioqe');
    ...
    const copyEles = await page.$$('[data-testid="msh-chat-segment-copy"]');
